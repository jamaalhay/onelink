...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  try {
    ...
    // Verified flag = customer with this email has at least one order
    // containing this product. Cheap signal-of-good-faith; not a strong identity
    // proof. Wrapped so the review still posts if the lookup fails (e.g.
    // unsupported filter on `items.product_id`).
    ...
    // Surface the real error in logs (and to the response in non-prod) so QA
    // can act on it. Default Medusa 500s strip the message.
    const e = err as Error;
    console.error("[reviews POST] handle=%s error: %s\n%s", handle, e.message, e.stack);
    res.status(500).json({
